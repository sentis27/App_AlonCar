...
```mermaid
graph TD
    GS008[Base de Personal y Tarifas] -.->|Fórmulas de búsqueda inestables| GS001[Planilla Horas]
    GS008 -.->|Enlaces externos rotos| GS003[Lista Trabajos]
    GS001 -->|Auditoría cruzada manual| GS010[Reconciliación]
    GS002[Planilla Materiales] -->|Separación manual de consumibles 1.5hs| GS010
    GS002 -.->|Cálculo de ajuste manual| GS007[Control Stock]
    GS006[Compras] -->|Control visual precario color morado| GS007
    GS001 -->|Cruce manual de datos| GS004[Resumen Gerencial]
    GS002 -->|Cruce manual de datos| GS004
    GS005[Terceros] -->|Congelamiento manual del dólar| GS004
    
    style GS004 fill:#551111,stroke:#ff0000,stroke-width:2px,color:#fff
    style GS010 fill:#551111,stroke:#ff0000,stroke-width:2px,color:#fff
```
...
```mermaid
graph TD
    Desc[Descripciones desde Lista de Trabajos] -->|Automático| CC{Motor de Consolidación App_AlonCar}
    H[Costo de Horas] -->|Automático| CC
    M[Costo de Materiales] -->|Automático| CC
    T[Costo de Terceros] -->|Automático| CC
    
    CC -->|Cálculo Automático: Exento MO / Gravado MAT| REV[Bandeja del Gerente: Revisión de Proforma]
    
    REV -->|Aprobar (1 Clic, sin ajustes manuales)| REMITO[Emisión de Remito en PDF]
    
    REMITO --> FACT[OT pasa a estado FACTURADO]
    
    FACT -->|Decisión Gerencial| CERRAR((Cerrar Definitivo: <br>Bloquea nuevas cargas))
    FACT -->|Decisión Gerencial| ABIERTO((Mantener Abierta: <br>Permite registrar costos extra post-factura))
    
    style CC fill:#003366,stroke:#00aaff,stroke-width:3px,color:#fff
    style CERRAR fill:#551111,stroke:#ff0000,stroke-width:2px,color:#fff
    style ABIERTO fill:#005500,stroke:#00ff00,stroke-width:2px,color:#fff
```
...
```mermaid
graph TD
    A[Retiros de Pañol] --> V{Embudo de Reconciliación Automático}
    B[Horas Cargadas] --> V
    
    V -->|Coincide| OK[Aprobado para Liquidación]
    V -->|Material sin horas cargadas| ALERTA[Alerta al Supervisor]
    
    ALERTA -.-> ASISTENCIA((El sistema muestra las <br>horas trabajadas por esa <br>persona en la semana <br>para contexto rápido))
    
    ASISTENCIA -->|Botón 1| R1(Re-imputar OT correcta)
    ASISTENCIA -->|Botón 2| R2(Cargar horas faltantes)
    
    style V fill:#003366,stroke:#00aaff,stroke-width:3px,color:#fff
    style ALERTA fill:#551111,stroke:#ff0000,stroke-width:2px,color:#fff
    style ASISTENCIA fill:#004488,stroke:#00aaff,color:#fff
```
