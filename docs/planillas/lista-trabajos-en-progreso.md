...
| **Cierre Comercial de OT (Data Locking Flexible)**: Al pasar una OT a estado "FACTURADO", el sistema debe ofrecer dos opciones: A) Cerrar Definitivo (bloquea estricta e irrevocablemente nuevas cargas de costo) o B) Mantener Abierta (permite seguir imputando costos adicionales de arrastre post-facturación para medir costo real vs facturado estimado). | WorkOrder.status + WorkOrder.allow_post_billing_costs | **Nueva regla de negocio para la App**. |
